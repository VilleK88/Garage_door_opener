...
int IPStack::write(unsigned char *buffer, int len, int timeout) {
    ...
    if (err != ERR_OK) {
        std::printf("Failed to write data %d\n", err);
        rv = -1;
    }
...
int IPStack::disconnect() {
    ...
    if (tcp_pcb != nullptr) {
        ...
        if (err != ERR_OK) {
            std::printf("close failed %d, calling abort\n", err);
